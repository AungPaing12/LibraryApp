///Type ID
const kVolumeInfoVOTypeID = 1;
const kImageLinkVOTypeID = 2;
const kAccessInfoTypeID = 3;
const kSaleInfoVOTypeID = 4;
const kSearchInfoVOTypeID = 5;
const kItemsVOTypeID = 6;
const kSearchResponseTypeID = 7;
const kBookVOTypeID = 8;
const kBookListsVOTypeID = 9;
const kResultsVOTypeID = 10;
const kHomeScreenResponseTypeID = 11;
const kBookHiveTypeID =12 ;

///Box Name
const kBoxNameForBookListVO = 'BoxNameForBookListVO';
const kBoxNameForBooksVO = 'BoxNameForBooksVO';
const kBoxNameForBookHiveVO = 'BoxNameForBookHiveVO';
const kBoxNameForShelf = 'BoxNameForShelf';
